...
class izioSeo
{
	...
	function izioSeo()
	{
		...
		$this->setLenght();
		$this->activateLog(get_option('izioseo_log') > 0);
	}
	...
	{
		$files = array(
			'file_keywords' => dirname(__FILE__) . '/' . trim($this->keywordList, '/'),
			'file_stopwords' => dirname(__FILE__) . '/stopwords.txt',
			'file_acronyms' => dirname(__FILE__) . '/acronyms.txt'
		);
		foreach ($data as $key => $value)
